...
import glob
import os
from stats import rbf, good_channels, processInput, expand_matrix, r2z, z2r, truncate
...
def main(fname, r, k_thresh):
    ...
    ## check if corr and fig directories exist
    ...
    if not os.path.isfile(os.path.join(full_dir, 'full_matrix_' + file_name + '_k' + str(k_thresh) + '_r' + str(r) + '.npz')):
        if not os.path.isfile(os.path.join(get_parent_dir(os.getcwd()), k_loc_name)):
            ...
            # R_full = np.load(os.path.join(get_parent_dir(os.getcwd()), full_loc_name))
            ...
            for i in files:
                try:
                    ...
                    C_K_subj, R_K_subj = good_channels(K_subj, R_subj, C_subj, k_thresh)
        ...
        if not R_K_subj == []:
            if np.shape(R_K_subj)[0] > 1:
                ...
                results = Parallel(n_jobs=num_cores)(
                    delayed(processInput)(coord, R_K_subj, RBF_weights, C_K_subj) for coord in inputs)
