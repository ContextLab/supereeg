...
def reverse_squareform(Cov_half):
    Cov_full = squareform(Cov_half, checks=False)
    Cov_full = Cov_full + np.eye(np.shape(Cov_full)[0])
    return Cov_full

def plot_cov(x, outfile=None, titles=None):
...
### this function returns a 6 column matirx (first 3 are x, y, z coordinates), 4th coloring, 5th size of node
def brain_net_viewer_format(R_full, subj_inds, electrode_ind):
    R_add_cols = np.append(R_full, np.zeros(np.shape(R_full)), 1)
...
### this function returns a 6 column matirx (first 3 are x, y, z coordinates) and 4th coloring
