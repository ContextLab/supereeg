...
import numpy as np
from stats import z2r, r2z, compile_corrs, density, tal2mni, normalize_Y, round_it, compile_nn_corrs, good_chans
from plot import  scatter_plot, brain_net_viewer_format, plot_times_series, pca_plot, plot_cov, reverse_squareform
...
def main(fname, electrode, r, k_thresh):
    ...
    ################# create master pandas dataframe compiling all the correlations ###################
    files = glob.glob(os.path.join(compare_dir, '*.npz'))
    ### to debug once compiled:
    ...
    # plt.savefig(outfile)
    # plt.clf()
    ########### Create timeseries for all electrodes for gif ##############################
    ...
    ### this requires nncorr to run first ######
    ...
    if subject_plots:
        ...
        if os.path.isfile(os.path.join(time_dir, 'corr_timeseries_T_' + file_name + '_' + electrode + '_' + str(k_thresh) + '_' + str(r) + '.npz')):
            ...
            print('total samples = '+ str(np.max(corrs[:,3])))
